...
class CacheGator {
    ...
    async loadChunk({ linesBuffer, query, batchCount, bytesCount }) {
        ...
        if (this.cacheType === "memory") {
            writeFileSync(`${this.tmpDir}/${this.keyPrefix}_${cacheKey}.layer.tmp`, JSON.stringify(chunkResult)); // persistent unless write a setTimeout if not use redis instead
        }
        else {
